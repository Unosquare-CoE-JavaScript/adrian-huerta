...
const useClapAnimation = ({ clapEl, countEl, clapTotalEl }) => {
  ...
  const [animationTimeline, setAnimationTimeline] = useState(timeline);
...
const ClapIcon = ({ style: userStyles = {}, className }) => {
  ...
  const clickedStyle = isClicked ? styles.checked : "";
  const classNames = [styles.icon, clickedStyle, className].join(" ").trim();
...
// Another way to implement te compound pattern is to expose an API to reference al the components
